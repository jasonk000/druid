 ...
 */
public class VirtualSegmentWrangler implements SegmentWrangler
{
  private static final String SEGMENT_ID = "virtual";

  @Override
  public Iterable<Segment> getSegmentsForIntervals(final DataSource dataSource, final Iterable<Interval> intervals)
  {
    final VirtualDataSource virtualDataSource = (VirtualDataSource) dataSource;

    return Collections.singletonList(
        new VirtualSegment<>(
            SegmentId.dummy(SEGMENT_ID),
            virtualDataSource.getCursorFactory(),
            virtualDataSource.getRowSignature()
        )
    );
  }
}
